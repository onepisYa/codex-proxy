...
use crate::config::{AccessKeyRole, ConfigHandle, with_config};
use crate::error::ProxyError;

#[derive(Debug, Clone)]
pub struct AuthenticatedKey {
    pub id: String,
    pub role: AccessKeyRole,
}
...
pub fn authenticate_request(
    ...
    match matched {
        Some(key) => {
            let role = key.effective_role();
            Ok(Some(AuthenticatedKey { id: key.id, role }))
        }
...
pub fn require_admin(key: Option<&AuthenticatedKey>) -> Result<(), ProxyError> {
    match key {
        Some(k) if k.role == AccessKeyRole::Admin => Ok(()),
        _ => Err(ProxyError::Auth(
            "Access key with role=admin required".into(),
        )),
    }
}
